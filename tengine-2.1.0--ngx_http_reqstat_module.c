...
ngx_http_reqstat_show_handler(ngx_http_request_t *r)
{
    ...
    ngx_rbtree_node_t            *rnode;//, *rsentinel; // libearxiong
    ...
    for (free = busy = NULL, i = 0; i < display->nelts; i++) {
	     ...
	     * add by libearxiong
 	     */
	    qq = ngx_queue_next(q);
            rnode = (ngx_rbtree_node_t *)
            ((u_char *) node - offsetof(ngx_rbtree_node_t, color));
	    ngx_shmtx_lock(&ctx->shpool->mutex);
            ngx_rbtree_delete(&ctx->sh->rbtree, rnode);
            ngx_slab_free_locked(ctx->shpool, rnode);
	    ngx_queue_remove(q);
	    ngx_shmtx_unlock(&ctx->shpool->mutex);
            q = qq;
        }
	// libearxiong
	//rnode = ctx->sh->rbtree.root;
	/*rsentinel = ctx->sh->rbtree.sentinel;
    	ngx_shmtx_lock(&ctx->shpool->mutex);
        for(;;) {
	    rnode = ngx_rbtree_min(ctx->sh->rbtree.root, &ctx->sh->sentinel);
	    if(rnode == rsentinel) break;
	    ngx_rbtree_delete(&ctx->sh->rbtree, rnode);
            ngx_slab_free_locked(ctx->shpool, rnode);
            rsentinel = ctx->sh->rbtree.sentinel;
	}
